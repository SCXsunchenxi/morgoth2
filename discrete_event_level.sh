...
#            --eval \
#            --model base_patch200_200 \
#            --task_model checkpoints/finetune_VW_onlyResampled/checkpoint-best.pth \
#            --dataset VW \
#            --nb_classes 1 \
#            --test_data_format pkl \
#            --eval_sub_dir /data/VW/processed_1second/test

#echo "$password" | sudo -S OMP_NUM_THREADS=1 $(which python) -m torch.distributed.run --nnodes=1 --nproc_per_node=2 --master_port=12 finetune_classification.py \
#            --abs_pos_emb \
#            --eval \
#            --model base_patch200_200 \
#            --task_model checkpoints/finetune_VW_SPIKES/checkpoint-best.pth \
#            --dataset VW_SPIKES \
#            --nb_classes 3 \
#            --test_data_format pkl \
#            --eval_sub_dir /data/VW_SPIKES/processed_1second/test
...
echo "$password" | sudo -S OMP_NUM_THREADS=1 $(which python) -m torch.distributed.run --nnodes=1 --nproc_per_node=2 --master_port=12 finetune_classification.py \
            ...
            --eval \
            --model base_patch200_200 \
            --task_model checkpoints/finetune_BIPD/checkpoint-best.pth \
            --dataset BIPD \
            --nb_classes 1 \
            --test_data_format pkl \
            --eval_sub_dir /data/BIPD/test_set
...
echo "$password" | sudo -S OMP_NUM_THREADS=1 $(which python) -m torch.distributed.run --nnodes=1 --nproc_per_node=2 --master_port=12 finetune_classification.py \
            ...
            --eval \
            --model base_patch200_200 \
            --task_model checkpoints/finetune_BIRD/checkpoint-best.pth \
            --dataset BIRD \
            --nb_classes 1 \
            --test_data_format pkl \
            --eval_sub_dir /data/BIRD/test_set